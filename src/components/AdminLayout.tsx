...
import { 
  ...
  PlusCircle, ShieldCheck, ClipboardList
} from 'lucide-react';
...
const AdminLayout: React.FC = () => {
  ...
  const menuItems = [
    ...
    { name: 'إدارة السيارات', icon: Car, path: '/admin/cars' },
    { name: 'طلبات العملاء', icon: ClipboardList, path: '/admin/leads' },
    { name: 'إدارة المسوقين', icon: Users, path: '/admin/marketers' },
    { name: 'التقارير المالية', icon: DollarSign, path: '/admin/reports' },
    ...
    <div className="min-h-screen bg-zinc-950 flex flex-col md:flex-row">
      ...
      <aside className="w-full md:w-64 bg-zinc-900 border-l border-white/5 p-6 space-y-8">
        <Link to="/" className="flex items-center gap-2 px-2">
          ...
          <span className="text-lg font-bold">لوحة المدير</span>
        </Link>
